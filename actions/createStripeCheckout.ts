...
import { createStudentIfNotExists } from "@/sanity/lib/student/createStudentIfNotExist";
import { clerkClient } from "@clerk/nextjs/server";
import { createEnrollment } from "@/sanity/lib/student/createEnrollment";

const validBaseUrl = baseUrl; // Fallback in case it's undefined
...
export async function createStripeCheckout(courseId: string, userId: string) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${validBaseUrl}/courses/${slug.current}`,
      cancel_url: `${validBaseUrl}/courses/${slug.current}?canceled=true`,
      metadata: {
        courseId: course._id,
        userId: userId,
      },
